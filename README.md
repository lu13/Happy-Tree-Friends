...
> **Summary**: An event-driven World of Warcraft Retail toolkit with safe merchant automation, a session ledger, friendly-name mode, and a customizable adventure HUD.
>
> **摘要**：事件驱动的《魔兽世界》正式服日常工具，提供安全的商人自动化、会话账本、友方仅姓名模式和可定制的冒险状态悬浮层。

## 描述

Happy Tree Friends 是适用于《魔兽世界》正式服 12.1 的轻量实用功能合集。它将高频日常操作收拢为可明确控制的功能：在商人处自动修理，可选优先使用公会维修额度；自动出售可出售的灰色物品，同时允许按物品链接或 ID 保护不应出售的物品。商人页面会显示本次登录内的修理支出、灰装收益和被跳过的受保护物品。

透明、可拖动的属性悬浮层可按需显示角色属性、装备耐久、背包空格、当前金币和延迟；低耐久或背包接近装满时会以颜色提示。插件还提供友方仅姓名模式、简体中文/英文界面、调试日志和可复制诊断报告。所有状态更新均由游戏事件触发，不使用 `OnUpdate` 循环或常驻扫描。

当前版本为 `0.5.0`，包含：

- 自动修理（可选个人金币，或公会银行优先）
- 自动售卖可出售的灰色物品，并支持按物品 ID 保护
- 本次登录会话账本：累计修理支出、灰装售卖收益和已跳过的受保护物品
- 友方仅姓名模式：保留友方玩家姓名并隐藏血条等姓名板信息
- 可锁定、可拖动并带高对比描边的透明角色属性悬浮层
- 每项属性独立显示/隐藏、文字颜色和全局字号设置
- 可按需在同一悬浮层显示装备耐久、背包空格、金币和延迟
- 低耐久与背包将满时变色提醒
- 精致的左右布局管理页与可选调试模式
- 根据客户端语言自动切换简体中文或英文
...
- `/htf protect <物品链接或 ID>`：保护指定灰色物品，不参与自动售卖。可先输入命令，再 Shift 点击物品链接。
- `/htf unprotect <物品链接或 ID>`：取消灰色物品保护
- `/htf protected`：查看已保护的物品 ID
- `/htf debug`：快速开关调试模式
- `/htf dump`：打开调试页并生成可复制的诊断报告
- `/htf clearlog`：清空持久化调试日志
- `/htf help`：显示命令帮助

插件设置页采用左侧分类、右侧功能内容的布局；为保护金币、物品与现有界面习惯，首次安装时自动修理、自动售卖灰色物品、公会维修和友方仅姓名模式默认关闭，属性悬浮层默认开启并锁定；冒险状态项目默认不显示，调试模式默认关闭。启用相应开关后立即生效。
...
“可选冒险状态”中的耐久、背包、金币和延迟也使用同一组显示开关、位置和字体设置。耐久低于 40% 或空背包格不超过 5 格会显示为琥珀色；耐久低于 20% 或空背包格不超过 2 格会显示为红色。数值只在进入世界、装备/背包/金币/区域变化、专精或属性变化及脱战后合并刷新，不使用常驻轮询。

位置、锁定状态、字号、每项可见性与颜色均保存在 `HappyTreeFriendsDB` 中，`/reload` 和重新登录后仍会保留。

### 灰装保护与本次会话账本

自动售卖开启后，默认仍使用客户端的批量出售能力。只要保存了至少一个受保护物品 ID，插件就会逐格处理可出售的灰色物品，确保受保护项不会卖出；商人页会实时显示本次登录期间的：

- 修理总额，以及个人金币和公会银行的分摊。
- 已售灰装数量和可读取物品售价时的累计收益。
- 被跳过的受保护物品及数量，最多列出 6 种，多余项合并计数。

账本只属于本次登录/插件加载会话，不写入 SavedVariables；受保护物品 ID 会保存。
...
- 自动售卖优先使用当前客户端的 `C_MerchantFrame.SellAllJunkItems()`；存在已保护物品 ID 时改用 `C_Container.GetContainerItemInfo()` 与 `C_Container.UseContainerItem()` 逐格出售未保护灰装，并通过 `C_Item.GetItemInfo()` 汇总可读取的卖店价。
...
没有 `OnUpdate` 循环、没有常驻背包扫描，也不监听战斗日志。商人功能只在打开商人窗口时执行一次；属性悬浮层只在启用时因进入世界、装备/专精/属性、耐久、背包、金币或区域变化及脱战后合并刷新一次；友方仅姓名模式只响应所管理的 CVar 变化，并合并到下一帧校正。被隐藏的属性和冒险状态不会调用对应读取 API。
...
3. 解锁属性悬浮层并拖动，重新锁定后确认背景透明、鼠标可正常点击其下方界面；再测试逐项隐藏、字号和颜色。按需启用耐久、背包、金币和延迟，确认低耐久/背包将满时变色。
4. 勾选调试模式，前往可修理商人；分别验证纯个人修理，以及有权限角色的公会银行优先修理。
5. 放入一个有售价的灰色物品后再次打开商人；确认它被自动出售。再用 `/htf protect` 加入另一件灰装，确认该物品被跳过、其他灰装仍售出，且商人页账本显示修理、收益和跳过项。
...
该测试覆盖默认设置、友方姓名 CVar 快照/应用/精确恢复/失败重试及旧客户端回退、HUD 透明/锁定/拖动与位置持久化、属性与冒险状态的逐项可见性、阈值变色、事件刷新、字体和颜色、Secret Value 安全降级、暴击口径、公会/个人维修分支、灰装保护、会话账本、商人操作与战斗后重试、持久化日志及诊断报告。它用于提前发现 Lua 逻辑回归，最终仍应以上述游戏内验收为准。
